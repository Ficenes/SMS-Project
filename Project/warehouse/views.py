...
def SiteToExcel(request):
    ...
    try:
        ...
        download = new_df.to_excel(excel_writer = "stores/static/warehouse_db.xlsx")
    return download
...
def show_warehouse_stock(request):
    ...
    if request.method == 'POST':
        ...
        if 'fileToUpload' in request.POST:
            ...
            to_del = Warehouse_Stock_info.objects.filter(id = request.POST['form_id'])
            to_del.delete()
            is_delete_form = True
        elif 'form_edit' in request.POST:
            my_record = Warehouse_Stock_info.objects.filter(id = request.POST['form_id']).first()
            ...
            my_record = Warehouse_Stock_info.objects.filter(id = request.POST['to_be_edit']).first()

...
def show_shop(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            req_value = float(
                request.POST['form_price']) * float(request.POST['quantity'])
            if Basket.objects.filter(sku=in_basket).count() != 0:
                Basket.objects.filter(sku=in_basket).update(
                    quantity=req_quantity +
                    float(Basket.objects.filter(sku=in_basket)[0].quantity),
                    value=req_value +
                    float(Basket.objects.filter(sku=in_basket)[0].value),
                )
            else:
